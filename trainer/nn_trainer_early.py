...
class nn_Trainer_early:
    '''nn_trainer class
    
    '''
    def __init__(self, model, modality, criterion, optimizer, config, device, train_dataloader, val_dataloader, uni=1):
        ''' Sets the class variables

        Args:
            model (_type:class_): the model to be trained, it can be either a unimodal or multimodal model
            modality (_type:str_): the modality of the model in the training process
            criterion (_type:class_): the loss function to be used during training
            optimizer (_type:class_): the optimizer to be used during training
            config (_type:class_): it contains the parameters according which the model will be trained
            device (_type:_): the device on which the model will be trained
            train_dataloader (_type:_): the dataloader for the training set
            val_dataloader (_type:_): the dataloader for the validation set
            uni (_type:int_): 1 means the model is trained for unimodal model, 0 means it is trained for multimodal model
